module.exports = {
    ...
    create: {
        ...
        item: function (q, m, o, t) {
            let nb_item = q.splitOnce(m, t.dico.sep);
            ...
            q.select('Items', { name: itemName }).then(rows => {
                ...
                if (!item) {
                    ...
                    if (nb < this.item.count) {
                        q.update('Items', { name: item.name }, { count: item.count - nb }).then(() => o.send(t.dico.on.remove.item, nb, m));
                    } else {
                        q.delete('Items', { name: item.name }).then(() => o.send(t.dico.on.remove.item, item.count, m));
                    }
                }
            })
        },
    ...
    give: function (q, m, o, t) {
        let item_chara = q.splitOnce(m, t.dico.sep.give);
        let nb_item = q.splitOnce(item_chara[0], t.dico.sep.item);
        ...
        q.select('Charas', { name: charaName }).then(rows1 => {
            ...
            q.select('Places', { name: placeName }).then(rows2 => {
                ...
                q.update('Charas', { name: chara.name }, { place: place.name }).then(
                    () => o.send(t.dico.on.move.new, chara.name, place.name)
                );
            });
        });
    },
    script: function (q, m, o, t) {
        let object_name = q.splitOnce(m, t.dico.sep.script.affect);
        q.insert('Do', { script: object_name[1], object: object_name[0] }).then(() => o.send(t.dico.on.script, object_name[1], object_name[0]));
    },
    unscript: function (q, m, o, t) {
        let object_name = q.splitOnce(m, t.dico.sep.script.affect);
        q.delete('Do', { script: object_name[1], object: object_name[0] }).then(() => o.send(t.dico.on.script, object_name[1], object_name[0]));
    },
